...
for card_id in ['ch1', 'ch2', 'ch3', 'ch4', 'ch5', 'ch6']:
    ...
    # Find all quiz blocks in this section
    quiz_pattern = r'<h3>🎯 随堂测验 Q\d+</h3>\s*<div id="q\d+_\d+">.*?</div>\s*</div>\s*</div>'
    quiz_blocks = re.findall(quiz_pattern, card_section, re.DOTALL)
    
    # Get quiz data for this unit
    quizzes = quiz_data.get(card_id, [])
    
    if len(quiz_blocks) != len(quizzes):
        print(f'{card_id}: Found {len(quiz_blocks)} quiz blocks, but have {len(quizzes)} quiz data')
        continue
    
    # Replace each quiz block
    new_section = card_section
    for i, (old_quiz, q_data) in enumerate(zip(quiz_blocks, quizzes)):
        # Build new quiz
        opts_html = ''
        ...
        new_quiz = f'''<h3>🎯 随堂测验 Q{i+1}</h3>
<div id="q{card_id[1:]}_{i+1}">
  <p><strong>Q{i+1}.</strong> {q_data['q']}</p>
  {opts_html}
  <div class="feedback" id="fb-{card_id[1:]}_{i+1}"></div>
</div>'''
        
        new_section = new_section.replace(old_quiz, new_quiz, 1)
    
    # Replace in content
    content = content[:card_start] + new_section + content[card_end:]
    
    print(f'{card_id}: Replaced {len(quiz_blocks)} quizzes')

# Fix div imbalance
div_open = content.count('<div')
div_close = content.count('</div>')
print(f'\nDiv: {div_open} open / {div_close} close')

if div_open > div_close:
    extra = div_open - div_close
    content = content.replace('</body>', '</div>\n' * extra + '</body>')
    print(f'Added {extra} closing divs')
elif div_close > div_open:
    extra = div_close - div_open
    body_end = content.rfind('</body>')
    for _ in range(extra):
        last_div = content.rfind('</div>', 0, body_end)
        if last_div > 0:
            content = content[:last_div] + content[last_div+6:]
            body_end = content.rfind('</body>')
    print(f'Removed {extra} closing divs')

# Recount
div_open = content.count('<div')
div_close = content.count('</div>')
print(f'Final div: {div_open} open / {div_close} close')
