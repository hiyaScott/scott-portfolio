# Scott Portfolio | 虾折腾

**个人作品集网站** - 游戏、工具、研究项目

🔗 **在线访问**: https://hiyascott.github.io/scott-portfolio/

---

## 🎮 游戏作品 (28个)

### 经典休闲类
| 游戏 | 类型 | 技术 |
|------|------|------|
| [妈妈计数器](https://hiyascott.github.io/scott-portfolio/games/mama-counter/) | 实用工具 | 纯Web |
| [扫雷](https://hiyascott.github.io/scott-portfolio/games/minesweeper/) | 经典游戏 | 🎮 Godot引擎 |
| [飞机大战](https://hiyascott.github.io/scott-portfolio/games/aircraft-war/) | 街机射击 | 纯Web |
| [贪吃蛇](https://hiyascott.github.io/scott-portfolio/games/snake/) | 经典街机 | 纯Web |

### 音乐节奏类
- [编钟模拟器](https://hiyascott.github.io/scott-portfolio/games/six-finger-midi/) - 15口钟完整音阶，文化体验
- [节奏指挥官](https://hiyascott.github.io/scott-portfolio/games/rhythm-commander/) - 精准打击节奏游戏
- [节奏跑酷](https://hiyascott.github.io/scott-portfolio/games/rhythm-parkour/) - 音乐+跑酷
- [颜色交响曲](https://hiyascott.github.io/scott-portfolio/games/color-symphony/) - 音乐+颜色匹配

### 策略动作类
- [时间切片](https://hiyascott.github.io/scott-portfolio/games/time-slice/) - 时间冻结策略

### 物理解谜类
- 引力弹弓、完美轨迹、量子迷宫、波函数坍缩

### 反应挑战类
- 形状变换、记忆迷宫、时间回溯

### 创意工具类
- 像素画板、热力膨胀、词语炼金、颜色交响曲

### 编程学习类
- Bot Coder、霓虹防线

**完整列表**: https://hiyascott.github.io/scott-portfolio/games/

---

## 🔬 研究项目

| 项目 | 内容 |
|------|------|
| [SRPG分析](https://hiyascott.github.io/scott-portfolio/research/srpg-analysis/) | 五款战棋游戏机制分析 |
| [乐器模拟器](https://hiyascott.github.io/scott-portfolio/research/instrument-simulator/) | 编钟+古琴Web模拟 |
| [游戏设计方法论](https://github.com/hiyaScott/scott-portfolio/tree/main/research/game-design) | TOP5评分体系与设计案 |

---

## 🛠️ 工具

- [认知负载监控](https://hiyascott.github.io/scott-portfolio/status-monitor/cognitive-status.html) - 实时状态面板

---

## 🐱 Jetton 能力图谱

探索 AI 助手的能力边界：
https://hiyascott.github.io/scott-portfolio/kimi-claw/

---

## 技术栈

- **前端**: HTML5 + CSS3 + JavaScript
- **游戏**: Canvas API / Web Audio API / Godot (Web导出)
- **部署**: GitHub Pages
- **监控**: Python + GitHub Actions

---

## 更新日志

- **2026-03-18**: 整合 game-design-portfolio (3游戏+3文档)
- **2026-03-18**: 添加4个经典游戏 (扫雷、飞机大战、贪吃蛇、妈妈计数器)
- **2026-03-17**: 认知负载监控 v5.34
- **2026-03-10**: 添加编钟模拟器设计案
- **2026-03-09**: 创建Jetton能力图谱

---

**Made with ❤️ by Scott**