...
if [ -f "$FAIL_COUNT_FILE" ]; then
    ...
    if [ "$fail_count" -gt 0 ]; then
        resolved_issue="恢复了因连续${fail_count}次失败而中断的数据同步"
    else
        resolved_issue="保持远程监控数据与本地实时同步"
    fi
else
    resolved_issue="保持远程监控数据与本地实时同步"
fi

commit_msg="monitor: update cognitive status at ${time_str}

解决了什么问题:
- ${resolved_issue}
- 确保GitHub Pages状态页面显示最新认知负载
...
Generated by: cognitive_push_v4.sh"
